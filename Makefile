...
build_web:
	docker run --volume ${PWD}/web:/web -w /web --volume /etc/passwd:/etc/passwd:ro --volume /etc/group:/etc/group:ro node:16 /bin/bash -c "npm install -g npm@8.19.1 && npm install && npm run build"
...
local:
	DOCKER_BUILDKIT=1 docker build --no-cache -t malenurse/frigate -f docker/Dockerfile .
